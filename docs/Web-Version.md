...
Available options depend on [site download restrictions](Download-Restrictions.md) configured by an administrator. Restricted choices are hidden from the menu; if all downloads are disabled, the Download control is not shown. By default, media downloads are disabled until an admin allows them.

When available:

* ==Image Sequence== or ==Video== will export the source media as a `.zip`
* ==Detections== / annotations will export a [VIAME `.csv`](DataFormats.md#viame-csv) (and related JSON/mask formats)
