...
def run_inference(
    ...
    sam2_config: Union[str, Path],
    sam2_checkpoint: Union[str, Path],
    frame_dir: Path,
    trackId: str,
    startFrame: int,
    trackingFrames: int,
    ...
    with torch.inference_mode(), torch.autocast(device, dtype=torch.bfloat16):
        ...
        if mask_location:
            ...
            frame_idx, object_ids, masks = predictor.add_new_mask(state, startFrame, ann_obj_id, torch_mask)
        else:
        ...
        for count, (frame_idx, object_ids, masks) in enumerate(
            predictor.propagate_in_video(state, startFrame, trackingFrames - 1), start=1
        ):
            if frame_idx < startFrame or count >= trackingFrames:
                continue
            for obj_id, mask in zip(object_ids, masks):
                save_and_record_mask(mask, output_dir, obj_id, frame_idx, rle_masks, track_data)
...
def get_mask_or_bbox(
    ...
    existing_tracks = gc.sendRestRequest('GET', '/dive_annotation/track', {'folderId': dataset_id})
    ...
    if track:
        ...
        matching_feature = next((f for f in features if f.get('frame') == start_frame), None)
        if matching_feature:
            if matching_feature.get('hasMask', False):
                media_results = gc.get(f'/dive_dataset/{dataset_id}/media')
                masks = media_results.get('masks', [])
                matching_mask = next(
                    (m for m in masks if m.get('frameId') == start_frame and m.get('trackId') == track_id),
                    None
                )
                if matching_mask:
                    mask_file_id = matching_mask.get('fileId')
                    mask_dir = working_directory / 'base_mask'
                    mask_dir.mkdir(exist_ok=True)
                    mask_path = mask_dir / 'mask.png'
                    gc.downloadFile(mask_file_id, str(mask_path))
                    mask_location = mask_path
...
def get_model_files(gc: girder_client.GirderClient, args, working_directory: Path) -> Tuple[Path, Path]:
    default_config = Path("/opt/SAM2/sam2.1_hiera_l.yaml")
    default_checkpoint = Path("/opt/SAM2/models/sam2.1_hiera_large.pt")
