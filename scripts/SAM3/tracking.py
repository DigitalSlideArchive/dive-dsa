...
def propagate_in_video(
    ...
    if inital:
        ...
        # Add initial prompt
        bounding_box_labels = [1 for _ in boxes]
        click.echo(boxes)
        response = predictor.handle_request(
            request=dict(
                type="add_prompt",
...
def inference_session(
    ...
    frame_height, frame_width = video_stats[0], video_stats[1]
    for i in range(len(boxes)):
        x, y, w, h = boxes[i]
        boxes[i] = [x / frame_width, y / frame_height, w / frame_width, h / frame_height]
    click.echo(boxes)
    boxes = [[0, 0, 1, 1]]
    propagated, loop_stopped_early, session_id = propagate_in_video(
        predictor,
        inital=True,
...
@click.option('--boxes', type=str, default="81, 23, 1105, 643", callback=int_list, help='Comma-separated list of bounding box coordinates: x1,y1,x2,y2')
@click.option('--num-frames', type=int, default=10)
...
def segment_video(video_path, start_frame, boxes, num_frames, output_dir, upload):
    """
    ...
    track_data, rle_masks = inference_session(predictor, video_path, box_list, start_frame, num_frames, track_data, rle_masks, video_stats=video_stats)
