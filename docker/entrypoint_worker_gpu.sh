...
chown -R ${USER_ID}:${GROUP_ID} /opt/dive/local

# `uv` canonicalizes the python interpreter path inside the venv. If
# `venv/bin/python3` is a symlink, `uv` may need execute/read permissions on
# the resolved target as well as on any parent directories.
chmod a+rx /opt /opt/dive /opt/dive/local 2>/dev/null || true
chmod -R a+rX /opt/dive/local/venv 2>/dev/null || true
chmod -R a+rX /opt/dive/local/.local 2>/dev/null || true
chmod a+rx /opt/dive/local/venv/bin/python3 2>/dev/null || true
if [[ -L /opt/dive/local/venv/bin/python3 ]]; then
  PY3_TARGET="$(readlink -f /opt/dive/local/venv/bin/python3 2>/dev/null || true)"
  if [[ -n "${PY3_TARGET}" && -e "${PY3_TARGET}" ]]; then
    # For uv-managed Python, the venv interpreter may point to a symlink
    # target under `/root/.local/share/uv/...` (because uv was invoked as
    # root during the image build). Non-root `DSA_USER` can't traverse `/root`
    # by default, so grant traversal to the resolved target path.
    if [[ "${PY3_TARGET}" == /root/* ]]; then
      chmod a+rx /root 2>/dev/null || true
      chmod a+rx /root/.local 2>/dev/null || true
      chmod -R a+rX /root/.local/share/uv 2>/dev/null || true
      chown -R ${USER_ID}:${GROUP_ID} /root/.local/share/uv 2>/dev/null || true
    fi
    chmod a+rX "${PY3_TARGET}" 2>/dev/null || true
    chown ${USER_ID}:${GROUP_ID} "${PY3_TARGET}" 2>/dev/null || true
  fi
fi
